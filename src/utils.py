...
def minimize_mask(bbox, mask, mini_shape):
    ...
    for i in range(mask.shape[-1]):
        ...
        #m = scipy.misc.imresize(m.astype(float), mini_shape, interp='bilinear') ###deprecated
        #m = Image.fromarray(m)
...
def expand_mask(bbox, mini_mask, image_shape):
    ...
    for i in range(mask.shape[-1]):
        ...
        h = y2 - y1
        w = x2 - x1
        #m = scipy.misc.imresize(m.astype(float), (h, w), interp='bilinear')  ### deprecated
        #m = Image.fromarray(m)
        #m = m.resize((h,w), resample = Image.BILINEAR)
        #m = (np.asarray(m)).astype(np.float32)
        m = resize(m, (h, w))
...
def display_top_masks(image, mask, class_ids, class_names, img_id, img_name,  limit=3):
    """Display the given image and the top few class masks."""
    to_display = []
    titles = []
    to_display.append(image)
    titles.append("{}({})({}x{})".format(img_name.split('.')[0], img_id, image.shape[0], image.shape[1]))
...
def read_image(filepath, target_size=(1024, 1024), space='RGB'):
    """Read an image from a file and resize it."""
    
    im = Image.open(filepath)
    img = im
    if target_size is not None:
        img = im.resize(target_size, resample = Image.BICUBIC) #NEAREST, BILINEAR
    if space == 'hsv':
        img = im.convert('HSV')
    return img

def read_train_data_properties(images_dir, annotations_dir, masks_dir):
    """Read basic properties of training images and masks"""
    import json
    
    
    json_file = open(os.path.join(annotations_dir,'all.json'))
    ...
    for image in coco_json['images']:
        ...
        mask_path = os.path.join(masks_dir, file_name_wo_ext+'.png')
        tmp.append(['{}'.format(im_id),file_name_wo_ext, im_H, im_W, 
                        img_ratio, img_path, mask_path])
        

    train_df = pd.DataFrame(tmp, columns = ['img_id', 'image_name', 'img_height', 'img_width',
                                            'img_ratio','image_path', 'mask_path'])
    return train_df


def load_raw_data(train_df, image_size=(1024, 1024), space = 'RGB',load_mask=True):
    """Load raw data."""
    from PIL import Image
    ...
    for i, filename in tqdm.tqdm(enumerate(train_df['image_path']), total=len(train_df)):
        ...
        if load_mask:
            mask = read_image(train_df['mask_path'].loc[i], target_size=image_size)
...
def cluster_images_by_hsv():
    ...
    print('Loading data')
    x_train_hsv = load_raw_data(image_size=None,space='hsv',load_mask=False)
    x_hsv = x_train_hsv
    ...
    kmeans = KMeans(n_clusters=7).fit(dominant_hsv)
...
def preview_image_and_masks(img_dir, msk_dir, mosaic_name):
    ...
    img = open_image(os.path.join(img_dir, id_ + '.jpg'))
    mask = open_mask(os.path.join(msk_dir, id_ + '.png'))
...
############################################################
#  Specific functions created for this project
############################################################


def import_file(ROOT_DIR, confirm = None):
    
    if confirm is not None:
        local_zip = os.path.join(ROOT_DIR, 'dataset2.zip')
        zip_ref = zipfile.ZipFile(local_zip, 'r')
        zip_ref.extractall(ROOT_DIR)
        zip_ref.close()
        
        return print('File was download and extracted at root path')
    
    
def recreate_data(ROOT_DIR, INPUT_DIR, confirm = None):
    ...
    if confirm is not None:
        ...
        if not os.path.exists(INPUT_DIR):
            try:
                ...
                os.mkdir('../considition-challenge/all')
                ...
                os.mkdir('../considition-challenge/all/Images')
                os.mkdir('../considition-challenge/all/Masks')
                os.mkdir('../considition-challenge/all/Annotations')
                os.mkdir('../considition-challenge/all/Percentages')
            except:
                print("Error while creating directories")
        return print('Directories at considition-challenge were recreated')
    else:
        return print('No changes were made at considition-challenge directories')

def define_dir(ROOT_DIR):
    ...
    all_dir = os.path.join(destination_dir, 'all/')
    all_images_dir = os.path.join(destination_dir, 'all/Images/')
    all_masks_dir = os.path.join(destination_dir, 'all/Masks/')
    all_percentages_dir = os.path.join(destination_dir, 'all/Percentages/')
    all_annotations_dir = os.path.join(destination_dir, 'all/Annotations/')
    
    return source_dir, destination_dir, images_dir, masks_dir, percentages_dir, \
        ...
        all_percentages_dir, all_annotations_dir
...
def summarize_percentages(percentages_dir, annotations_dir):
    ...
    for item in data['images']:
        tmp = {}
        tmp['imageName'] = item['file_name']
        tmp['image_id'] = item['id']
        id_to_name_list.append(tmp)
    ...
    for filename in json_names:
        ...
        file_name_wo_ext = os.path.splitext(filename)[0]
        file_name = file_name_wo_ext +'.jpg'
        base_name = file_name_wo_ext.split('_')[0]
    
        results = {}
        results['baseName'] = base_name
    ...
    return percentages, df_results, list_exclude_files, id_to_name_list

def create_train_test_split(df_complete, X , y , split = 0.15, seed = 4242):
    ...
    all_list_jpg = df_complete.imageName.values.tolist()


    train_list = X_train.image.values.tolist()
    val_list = X_test.image.values.tolist()
    all_list = df_complete.image.values.tolist()
    ...
    print('Train : {} different types' .format(y_train.nunique()))
    print('Valid : {} different types' .format(y_test.nunique()))
    
    return train_list_jpg, val_list_jpg, all_list_jpg, train_list, val_list, all_list
